<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        if ($_POST['type'] == "createnew") {
            ...
            $selectedprofileres = $query->fetch_assoc();
            header('Cache-Control: no cache');
        } elseif ($_POST['type'] == "create") {
          header('Cache-Control: no cache');
            $profileid = NULL;
            $query = $con->query("SELECT * FROM profiles WHERE citizenid = ".$con->real_escape_string($_SESSION["citizenid"]));
            $selectedprofileres = $query->fetch_assoc();
            ...
            $insert = $con->query("INSERT INTO reportsres (title,author,profileid,report,laws,created) VALUES('".$con->real_escape_string($_POST['title'])."','".$con->real_escape_string($_POST['author'])."','".$con->real_escape_string($profileid)."','".$con->real_escape_string($reportnote)."', '".json_encode($lawids)."',".time().")");
            header('Cache-Control: no cache');
            ...
            $query = $con->query("SELECT * FROM reportsres WHERE id = ".$con->real_escape_string($_POST['reportid']));
            $selectedreportres = $query->fetch_assoc();
            $laws = json_decode($selectedreportres["laws"], true);
            header('Cache-Control: no cache');
            $lawsedit_array = [];
            $totalprice = 0;
            $totalmonths = 0;
            ...
            $profile = $con->query("SELECT * FROM profiles WHERE id = ".$con->real_escape_string($selectedreportres['profileid']));
            ...
            $reportnote = nl2br($_POST["report"]);
            $update = $con->query("UPDATE reportsres SET title = '".$con->real_escape_string($_POST['title'])."', author = '".$con->real_escape_string($_POST['author'])."', profileid = ".$con->real_escape_string($profileid).", report = '".$con->real_escape_string($reportnote)."', created = ".time()." WHERE id = ".$_POST['reportid']);
            if ($update) {
      ...
      <div class="tooltip">
        ...
        <main role="main" class="container">
            ...
            <div class="createreport-container">
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "edit" && !empty($selectedreportres)) { ?>
                    <form method="post">
                        ...
                        <input type="hidden" name="reportid" class="form-control login-pass" value="<?php echo $selectedreportres["id"]; ?>" placeholder="" required>
                        <div class="input-group mb-3">
                            <input type="text" name="title" class="form-control login-user" value="<?php echo $selectedreportres["title"]; ?>" placeholder="titel" required>
                        </div>
                        ...
                        <?php } ?>
                        <?php $report = str_replace( "<br />", '', $selectedreportres["report"]); ?>
                ...
                <?php } else { ?>
                    <form method="post">
                        ...
                        <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "createnew") { ?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="<?php echo $selectedprofileres["citizenid"]; ?>" placeholder="Koppel BSN (Mag leeg zolang het niet om 1 persoon draait)">
                            </div>
                        <?php } else {?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="<?php echo $_SESSION["citizenid"]; ?>" placeholder="Koppel BSN (Mag leeg zolang het niet om 1 persoon draait)">
                            </div>
                        <?php } ?>
                        <div class="">
                        <textarea class="ck-content" name="report" id="editor">
...
---
<li>BETREFT: <strong><?php echo $_SESSION["recherchenaam"] ?></li></strong>
<li>Datum en tijd: </li>
<li>Notitie: </li>
---
<li>Bewijsstuk A: </li>
...
<li>Ik, verbalisant <?php echo $firstname . " " . substr($lastname, 0, 1); ?>, heb op <?php echo date("d/m/Y") ?> rond <?php echo date("H:i") ?> uur het bovenstaande bewijs bij elkaar gelegd en ben op de volgende conclusie gekomen.</li>
---
<li>Ik, verbalisant <?php echo $firstname . " " . substr($lastname, 0, 1); ?>, kom, gelet op de bovenstaande, tot de volgende conclusie. Aan <strong><?php echo $_SESSION["recherchenaam"] ?> </p></strong> &nbspdienen de volgende punten te worden opgelegd:</li>
<li>Punten: </li>
