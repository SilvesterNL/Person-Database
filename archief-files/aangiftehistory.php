<?php
    ...
    $aanfigtes = $con->query("SELECT * FROM aangifte_old ORDER BY created DESC");
    $aangiftes_array = [];
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        if ($_POST['type'] == "show") {
            $_SESSION["aangifteselector"] = $con->real_escape_string($_POST['selector']);
            Header("Location: aangiftevieuwerold");
        }}

        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            if ($_POST['type'] == "search") {
                $result = $con->query("SELECT * FROM aangifte_old WHERE concat(' ', naamslacht, ' ') LIKE '%".$con->real_escape_string($_POST['search'])."%' OR concat(' ', locatie, ' ') LIKE '%".$con->real_escape_string($_POST['search'])."%' OR concat(' ', agent, ' ') LIKE '%".$con->real_escape_string($_POST['search'])."%' ORDER BY created DESC");
                $search_array = [];
    ...
    <div class="sidebar-footer">
      ...
      <div class="tooltip">
        ...
        <main role="main" class="container">
            ...
            <h5 class="panel-container-title">Verlopen Aangiftes:</h5>
                    <div class="panel-list">
                    <form method="post" class="form-inline ml-auto">
                        <input type="hidden" name="type" value="search">
                        <div class="md-form my-0">
                            <input class="form-control" name="search" type="text" placeholder="Zoek een oud aangifte.." aria-label="Search">
                        </div>
                    ...
                    <div class="search-panel">
                        <h5 class="panel-container-title">Gevonden Aangiftes..</h5>
                        <div class="panel-list">
                            <?php if (empty($search_array)) { ?>
                                <p>Geen Aangiftes gevonden..</p>
                            <?php } else { ?>
                                <?php foreach($search_array as $aangiftezoek) {?>
                                    <form method="post">
                                        <input type="hidden" name="type" value="show">
                                        <button type="submit" class="leftalign btn btn-panel panel-item">
                                            <h5 class="panel-title"><?php echo $aangiftezoek['naamslacht']; ?></h5>
                                            <p class="panel-author">Datum: <?php echo $aangiftezoek['datum']; ?></p>
                                        </button>
                                    </form>
                                <?php }?>
                            <?php } ?>
                ...
                <?php } ?> <br>
                <h5 class="panel-container-title">Recent verlopen aangiftes:</h5>
                    <?php if(!empty($aangiftes_array)) { ?>
                        <?php foreach($aangiftes_array as $aangiftedata) {?>
                            <form method="post" action="">
                                <input type="hidden" name="type" value="show">
                                <input type="hidden" name="selector" value="<?php echo $aangiftedata['created']; ?>">
                                <button type="submit" class="aangifte btn btn-panel panel-item" style="text-align:left!important;">
                                    <h5 class="panel-title"><strong><?php echo $aangiftedata['naamslacht']; ?></strong></h5>
                                    <p class="panel-author">Datum: <?php echo $aangiftedata['datum']; ?></p>

                                </button>
                            </form>
                        <?php }?>
                    <?php } else { ?>
                            <p>Geen verlopen aangiftes..</p>
                    <?php } ?>
            </div>
        </main><!-- /.container -->
